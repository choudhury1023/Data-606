...
***The relationship looks linear. Yes I would be comfortable using a linear model to predict the number of `runs` if I know a team's `at_bats`.***
...
***The two variables appear to have a positive correlation, as `at_bats` increase the `runs` tend to increase too. The relationship seems moderately strong with few interseting outliers.***
...
***Smallest sum of squares I got was 142024.2. It is still higher than the sum of squares we got in exercise 2.***
...
***Home run has storng correlation with runs scored. For every home run scored teams run increased  by `r m2$coefficients[2]`, which is the slope of the regression line.***
...
\[
  \hat{y} = `r -2789.2429 + 0.6305 * 5578`
\]


***The manager would predict 727.6861 or 728 runs. With actual 5,579 at-bats runs are 713, the manager would over estimate by `r 728 - 713`***
...
***No other pattern apparent but linear relationship between at-bats and runs.***


7.  Based on the histogram and the normal probability plot, does the nearly 
    normal residuals condition appear to be met?
    
***Yes, it does. Although the histogram is a little misleading with slight right skewness and not very symmetrical, the Q-Q plot represent the nearly normal residuals condition better.***
...
***I think `hits` will be a good predictor of `runs`, so my chosen traditional variable is `hits`.***
...
***Yes, there seem to be a linear relationship between `hits` and `runs'.***
...
***My variable `hits` is better than `at_bats` because 37.3% of the variability in runs is explained by `at_bats` where significantly more, about 64.2% of the variability in runs is explained by `hits`.***
...
***`bat_avg` best predicts `runs`, with a R$^2$ value of 65.6% and correlation value `r cor(mlb11$runs, mlb11$bat_avg)`.***
...
***The three newer variables more effective at predicting runs that the old variables. The R$^2$ value for `new_onbase` is 84.9%, `new_slug` is 89.7% and `new_obs` is 93.5%. The plots below indicates a strong linear relationship. Of all ten variables the best pridctior of runs is `new_obs`.***
