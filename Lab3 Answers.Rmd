...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter __B_.
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    The stepwise pattern appears because of repeated values, one reason could be     rounding up of values like age.
...
```{r female-kne.di-mean-sd, eval=TRUE}
fkne.dimean <- mean(fdims$kne.di)
fkne.disd   <- sd(fdims$kne.di)
```

Skwed to the right, ends are above normal line.
