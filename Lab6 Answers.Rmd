...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    ***Sample statistics. The numbers are bsed on poll or sample data.***
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    ***We must assume the sample observations were independent, the size was less
    then 105 of the population and has atleast 10 success and 10 failure.***
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    ***Each row of table 6 correspond to the results of each country,(sample
    size, percent of religious person, percent of non-religious person, percent
    convinced atheist, percent of no response)***
...
round(nrow(us12atheist)/nrow(us12),2)
```
***The rounded figure is same as table 6, 5%.***
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    ***The conditions for inference to construct a 955 confidence interval for
    the proportion of atheists in the United States in 2012 should be,
    independent observations, sample size less then 10% of population and minimum
    of 10 success and 10 failures. The sample size is 1002 which is large enough
    and less then 10% of United State's population and the sucess is at
    `r 1002*.05` more then 10.***
...
```{r atheists_rus12, eval=TRUE, tidy=TRUE}
rus12<-subset(atheism,nationality=="Russian Federation"& year =="2012")
inference(rus12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
***Russian Federaton 2012 sammple size large enough at 1000 and less then 10% of 
the population which means it is independent and success at 60 and failure at 940 are both more then 10.***

***Mergin of error***
```{r mergin_of_error_rus12, eval=TRUE}
(0.0747-0.0453)/2
```
   
##China 2012
```{r atheists_chi12, eval=TRUE, tidy=TRUE}
chi12<-subset(atheism,nationality=="China"& year =="2012")
inference(chi12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
***China 2012 sammple size large enough at 500 and less then 10% of 
the population which means it is independent and success at 235 and failure at 265 are both more then 10.***

***Mergin of error***
```{r mergin_of_error_chi12, eval=TRUE}
(0.5137-0.4263)/2
```
...
***`me` incrases as the `p` increases and reaches to its maximum p reaches 0.5 and starts decreasing as `p` increases even more and reaches to 0 as `p` reaches 1.***
...
range(p_hats)
```
***The smpling distribution of sample proportion is near normal and centered near 0.099, spread is between 0.07 to 0.13 and is unimodal.***
...
```
***The distribution stays normal, as the n value increases the spread decreases and as the p value shifts the center.***

```{r reset-plot-window, eval=TRUE}
par(mfrow = c(1, 1))
```
...
me
```
***The margin of error is almost similar, the diffrence of 1040 and 400 is not that significant but the p value of 0.1 and 0.02 is significant. One isssue is noticable for Ecuador is 2% of 400 is only 8 so it does not have the minimum success of 10.***
...
```{r sp05-sp12, eval=TRUE, tidy=TRUE}
...
```
***In 2005 Spain's atheism index at 95% concidence interval was 0.083 - 0.1145 and in 2012 it was  0.0734 - 0.1065 and since they overlap I would conclude thre is not a staistically significiant evidene of change in atheism.***
...
```
***In 2005 United States's atheism index at 955 confidence interval was between 0.0038 - 0.0161 and in 2012 it was 0.0364 - 0.0634 and since they do not overlap I would conclude United States has seen a chang in its atheism index.
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    ***Would assume there is at significance level of  0.05, there 5% chance of
    type 1 error, where we wronglfully reject the null hypothesis.***
...
-   Suppose you're hired by the local government to estimate the proportion of 
    ...
    ***Margin of error, me <- 2 * sqrt(p * (1 - p)/n)
